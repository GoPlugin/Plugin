...
contract VRFCoordinatorV2 is VRF, ConfirmedOwner, TypeAndVersionInterface {
  ...
  mapping(address /* oracle */ => uint96 /* PLI balance */) private s_withdrawableTokens;
  ...
  int256 internal s_fallbackWeiPerUnitPli;
    ...
    onlyOwner()
  {
    ...
    s_config = Config({
      ...
      fulfillmentFlatFeePliPPM: fulfillmentFlatFeePliPPM,
    ...
    )
  {
    ...
    return (
      ...
      config.fulfillmentFlatFeePliPPM,
      config.maxGasLimit,
    ...
    nonReentrant()
  {
    ...
    uint96 payment = calculatePaymentAmount(startGas, s_config.gasAfterPaymentCalculation, s_config.fulfillmentFlatFeePliPPM, tx.gasprice);
    ...
    )
  {
    ...
    uint256 paymentNoFee = 1e18*weiPerUnitGas*(gasAfterPaymentCalculation + startGas - gasleft()) / uint256(weiPerUnitPli);
  ...
  // want to keep the subId, but withdraw all the pli.
